...
for i = 1:numChannel
    ...
    try
        ...
        burstPCARaw{i,1} = burstPCARaw{i,1};
        burstPCAOnOff{i,1} = burstPCAOnOff{i,1}; % get all the bursts with the same lengths, which is the maximum burst length of that channel in both classes
    end
    ...
    try
        ...
        cutoffLocs(i,1) = nan;
        burstPCA{i,1} = burstPCARaw{i,1}(1:end,:);
    end
