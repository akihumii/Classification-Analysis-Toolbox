...
for i = 1:numColumn
    ...
    for j = 1:numLocs(i,1)
        burst{j,1} = data(transpose((floor(-windowSize(1,1)*samplingFreq)+startLocs(j,ch)):endLocs(j,ch)),i);
        xAxisValuesTemp = transpose(1+floor(-windowSize(1,1)*samplingFreq):endLocs(j,ch)-startLocs(j,ch)+1); % create an array of x axis values for burst plotting
        % get the correct time axis, because the xAxisValuesTemp might
        % start from negative value, so need to compensate that part
        minusLocation = 1-xAxisValuesTemp(1,1);
        timeTemp = timeAxis-timeAxis(minusLocation);
