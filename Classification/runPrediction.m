    ...
    for i = 1:iterTest % test the classifier
        ...
        for j = 1:numFeatureSet
            for k = 1:numChannel
                ...
                predictClass{j,k,i}(:,n) = predict(classifierOutput.classificationOutput{j,1}.Mdl{k},groupedFeature{j,k,i}(:,end).testing); % get the prediction
                
                accuracyTemp{j,k,i}(n,:) = calculateAccuracy(predictClass{j,k,i}(:,end),ones(size(predictClass{j,k,i}(:,end)))*classTemp);
            end
        end
