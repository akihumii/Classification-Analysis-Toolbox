function [] = saveMClustInfo(signalClassification, signal, parameters)
...
for i = 1 : numChannel
    for j = 1 : numSpikes
        switch parameters.dataToBeDetectedSpike
            case 'dataFiltered'
...
for i = 1:numChannel
    for j = 1:numSpikes
        wvInterpolated(j,i,:) = interpolateData(wv(j,i,:), parameters.interpolationNumber);
    end
end
