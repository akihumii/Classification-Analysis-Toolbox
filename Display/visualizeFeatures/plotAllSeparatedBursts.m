...
for i = 1:numChannel
    ...
    for numSaveSubplot = 1:numArrayFullBursts
        ...
        if displayInfo.saveReconstruction % save combined figures
            savePlot(path,'Combined Full Length Bursts Subplots',plotFileName,['Combined Full Length Bursts Suplots ',num2str(numSaveSubplot)])
        end
    end
    
    if flagPlotSingleColumn && displayInfo.saveReconstruction
        numSaveSubplot = numSaveSubplot + 1;
        savePlot(path,'Combined Full Length Bursts Subplots',plotFileName,['Combined Full Length Bursts Suplots ',num2str(numSaveSubplot)],fFullBursts(arrayFullBursts{end,1}))
    end
    
    if flagPlotLessBurstsColumn && displayInfo.saveReconstruction
        numSaveSubplot = numSaveSubplot + 1;
        savePlot(path,'Combined Full Length Bursts Subplots',plotFileName,['Combined Full Length Bursts Suplots ',num2str(numSaveSubplot)],fFullBursts{end,1})
    end
